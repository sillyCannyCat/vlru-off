...
class Initiator(models.Model):
    ...
    initiator_name = models.CharField(length=256)

class Source(models.Model):
    source_id = models.IntegerField(primary_key=True)
    source_name = models.CharField(length=256)
...
class Street(models.Model):
    ...
    street_name = models.CharField(length=128)

class District(models.Model):
    district_id = models.IntegerField(primary_key=True)
    district_name = models.CharField(lenght=64)

class FolkDistrict(models.Model):
    folk_district_id = models.IntegerField(primary_key=True)
    folk_district_name = models.CharField(length=128)

class BuildingType(models.Model):
    building_type_id = models.IntegerField(primary_key=True)
    building_type_name = models.CharField(length=128)

class City(models.Model):
    city_id = models.IntegerField(primary_key=True)
    city_name = models.CharField(length=128)
